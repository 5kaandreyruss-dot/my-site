...
const TASKS3=[
 ...
 ()=>{const a=R(2,9),b=R(2,9);
   return{q:`Реши пример: ${a} × ${b} = ?`,a:a*b,h:"Вспомни таблицу умножения."};},
 ()=>{const b=R(2,9),per=R(2,9),k=b*per;
   return{q:`Реши пример: ${k} ÷ ${b} = ?`,a:per,h:`Подумай, на что нужно умножить ${b}, чтобы получить ${k}.`};},
 ()=>{const side=R(2,9);
   return{q:`Периметр квадрата со стороной ${side} см. Чему равен периметр?`,a:4*side,h:"Периметр квадрата = сторона × 4."};},
 ()=>{const a=R(2,9),b=R(2,9);
   return{q:`Длина прямоугольника ${a} см, ширина ${b} см. Чему равен периметр? (2×(длина+ширина))`,a:2*(a+b),h:"Периметр прямоугольника = 2 × (длина + ширина)."};},
 ()=>{const n=R(1,9)*100;
   return{q:`Сколько сотен в числе ${n}?`,a:n/100,h:"Раздели число на 100."};},
 ()=>{const a=R(2,9),parts=[2,3][R(0,1)],n=parts*a;
   return{q:`В корзине ${n} груш. Взяли 1/${parts} часть. Сколько груш взяли?`,a:n/parts,h:"Раздели число груш на знаменатель доли."};},
 ()=>{const a=R(100,500),b=R(100,500),c=R(50,a+b-10);
   return{q:`На складе было ${a} кг картошки и ${b} кг моркови. Продали ${c} кг овощей. Сколько килограммов овощей осталось?`,a:a+b-c,h:"Сначала сложи, сколько было всего, потом вычти проданное."};},
];
